<?php
	...
	$queryCheck = "SELECT a.*,b.* from transaction_sm a left join customer b on a.cust_id=b.cust_id where a.ref_no = '$query' and status=2";
	...
	if ($con->getRowCount() == 1) {
		...
		die("<div class='alert alert-warning'>
			...
			<strong>Warning! </strong> Reference '$query' was already claimed. Please inform the Main Branch.<br>
			info received : ".$result['date']."  claimed : ".$result['date_claimed']." by :".$result['off_cust_id']." 
			".$result['fname']." ".$result['mname']." ".$result['lastname']."
		</div>");
