	...
	<div id='selfForm'>
		...
		<div id='resultArea'>				
			...
			<?php
				...
				$query = $con->query("select * from sms_archive where address='SMARTMoney'");
				$rowCount = $con->getRowCount();

				$sms = $con->query("select * from sms_archive where address='SMARTMoney' limit ".($page*100)." ,100 ");
				...
				for($i=0; $i<($rowCount / 100); $i++)
				{
						...
						echo "<li><a  id='link".$i."' class='linkbtn' role='button' href='smsArchive.php?page=$i'>".($i+1)."</a></li>";
